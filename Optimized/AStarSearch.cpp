...
bool AStarSearch::search(string start, string goal) {
    ...
    while (!frontier.empty()) {
        ...
        if (current.name == goal) { // check if goal
            goalFoundOperations(current, goal);
            return true;
        }
        if (cities.getCityEntry(current.name)->isVisited()) {continue;} // don't add neighbors if city has been visited
        else {
            addNeighbors(current, end);  
        }
        // sets F, G, and parent on city graph if not expanded already
...
double AStarSearch::getHVal(double lat1, double lon1, double goalLat, double goalLon) {
    double dlat = (goalLat * M_PI/180) - (lat1 * M_PI/180);
    double dlon = (goalLon * M_PI/180) - (lon1 * M_PI/180);
    double a = pow(sin(dlat/2), 2) + cos(lat1 * M_PI/180) * cos(goalLat * M_PI/180) * pow(sin(dlon/2), 2);
...
void AStarSearch::reset() {
    while (!frontier.empty()) { // empty fronter
        frontier.pop();
    }
    nodesGenerated = totalDistance = nodesLeft = 0; // reset search info
...
/*  
 * addNeighbors()
 * Purpose: adds any unvisited neighbors of fNode to frontier
...
void AStarSearch::addNeighbors(fNode current, SearchNode* end) {
    ...
    for (int i = 0; i < numNeighbors; i++) { // for each neighbor
        ...
        if (!s->isVisited()) { // if neighbor hasn't been visited
